...
class StudentsAdminController extends Controller
{
    ...
    public function infoStudent(Student $student){
        ...
        ->with('tests')
        ->first();
    ///Queda pendiente para cuando se registre en la tabla result el resultado del último test
        if( $s->result == null){
        ...
        if( $s->tests->contains(Test::find(2))){
        ...
        //     $answerTrayectoryTest = (array) json_decode(stripslashes($s->tests[1]->pivot->answers));
        //     $answerVocationalTest = (array) json_decode(stripslashes($s->tests[0]->pivot->answers));
